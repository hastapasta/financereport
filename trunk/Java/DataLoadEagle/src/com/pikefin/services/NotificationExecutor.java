  ...
  public void run() {
    ...
    while (!bDone) {
      try {
        ...
        int nCurTargetId = 0;
        String strCurEmail = "";
        Iterator<Object> it = rs1.iterator();
        ...
        while (true) {
          if (bFirstPass == false) {
            ...
            tmpHM.put(Constants.NotificationData.NOTIFICATION_DATE_TIME_FIRED,
                String.valueOf(la.getDateTimeFired()));
            tmpHM.put(Constants.NotificationData.NOTIFICATION_TICKER,
                e.getTicker());
            tmpHM.put(Constants.NotificationData.NOTIFICATION_TIME_EVENT_NAME,
                te.getName());
            tmpHM.put(Constants.NotificationData.NOTIFICATION_DIFF,
                la.getDiff()+"");
            tmpHM.put(Constants.NotificationData.NOTIFICATION_BEFORE_VALUE,
                bfd.getValue()+"");
            tmpHM.put(
                Constants.NotificationData.NOTIFICATION_BEFORE_DATE_COLLECTED,
                bfd.getDateCollected().toString());
            tmpHM.put(Constants.NotificationData.NOTIFICATION_AFTER_VALUE,
                afd.getValue()+"");
            tmpHM.put(
                Constants.NotificationData.NOTIFICATION_AFTER_DATE_COLLECTED,
                afd.getDateCollected().toString());
            tmpHM.put(
                Constants.NotificationData.NOTIFICATION_CAKE_EDIT_ALERT_URL,
                ApplicationSetting.getInstance().getEmailCakeUrl() +
                Constants.NotificationData.NOTIFICATION_CAKE_EDIT_ALERT_SUB_URL
                + a.getAlertId());
        ...
        ApplicationSetting.getInstance().getStdoutwriter()
          .writeln("Problem with sql statement.", Logs.ERROR, "DG30.999");
        ApplicationSetting.getInstance().getStdoutwriter()
          .writeln(sqle);
      }
      try {
        ...
        ApplicationSetting.getInstance().getStdoutwriter()
          .writeln("NotificationExecuter Thread interrupted. Terminating.",
              Logs.ERROR, "DG30.999");
        return;
      }
    }
