...
public class BrokerExecutor extends Thread {
  ...
  public void getTriggeredJobs(){
    ...
    try {
      ...
      List<Schedule> schedules =
          scheduleService.loadAllSchedulesForBrokerExecutor();
      for (Schedule tempSchedule:schedules) {
        //check if task is being excluded
        if  (tempSchedule.getTask()!=null
            && taskService.isTaskExcluded(tempSchedule.getTask().getTaskId())){
          continue;
        }
        ...
        if (!bInQ) {
          QueuedJob queueNewJob=new QueuedJob(tempSchedule,
              tempSchedule.getTask(), tempSchedule.getRepeatType(),
              tempSchedule.getPriority(), Calendar.getInstance(),
              tempSchedule.getVerifyMode());
          waitingJobList.add(queueNewJob);
        }
        if(RepeatTypeEnum.RUNONCE.toString()
            .equals(tempSchedule.getRepeatType().getType())) {
  ...
  // Check if the Queued job is already not running then it initiates the job
  // and remove it from waiting job list
  // @param nPriority
  // @return
  // @throws DataAccessException

  public DataGrabExecutor initiateJob(int nPriority)
      throws DataAccessException  {
    ...
    for (int i=0;i<waitingJobList.size();i++) {
      ...
      for (int j=0;j<runningJobsArray.length;j++) {
        if (runningJobsArray[j] != null)  {
          if (runningJobsArray[j].getCurrentTask().getTaskId()
              == task.getTaskId()) {
            ApplicationSetting.getInstance().getStdoutwriter()
      ...
      if (isAlreadyRunning == false) {
        if (waitingJobList.get(i).getPriority() < nPriority) {
          ApplicationSetting.getInstance().getStdoutwriter()
          .writeln("Not initiating schedule because one thread is reserved " +
        ...
        dg = new DataGrabExecutor(task,0,waitingJobList.get(i).getSchedule(),
            waitingJobList.get(i).getRepeatType(),
            waitingJobList.get(i).isVerifyMode(),
            waitingJobList.get(i).getPriority());
        dg.start();
        ApplicationSetting.getInstance().getStdoutwriter()
  ...
  public void updateJobStats(DataGrabExecutor dg) {
    if (ApplicationSetting.getInstance().isLoadHistoricalData()!= true) {
      try {
        ...
        logTask.setJobProcessStart(jobProcessStart!=null?
            jobProcessStart.getTime():null);
        Calendar jobProcessEnd=dg.getTaskMetric(TaskMetricsEnum.JOB_END);
        logTask.setJobProcessEnd(jobProcessEnd!=null?
            jobProcessEnd.getTime():null);
        Calendar alertProcessStart =
            dg.getTaskMetric(TaskMetricsEnum.ALERT_START);
        logTask.setAlertProcessStart(alertProcessStart!=null?
            alertProcessStart.getTime():null);
        Calendar alertProcessEnd=dg.getTaskMetric(TaskMetricsEnum.ALERT_END);
        logTask.setAlertProcessEnd(alertProcessEnd!=null?
            alertProcessEnd.getTime():null);
        Calendar stage1Start=dg.getTaskMetric(TaskMetricsEnum.STAGE1_START);
        logTask.setStage1Start(stage1Start!=null?
            stage1Start.getTime():null);
        Calendar stage1End=dg.getTaskMetric(TaskMetricsEnum.STAGE1_END);
        logTask.setStage1End(stage1End!=null?
            stage1End.getTime():null);
  ...
  public void checkMessageCount() {
    ...
    if (calCurrent.get(Calendar.DAY_OF_YEAR) >
      threadStartDate.get(Calendar.DAY_OF_YEAR)) {
