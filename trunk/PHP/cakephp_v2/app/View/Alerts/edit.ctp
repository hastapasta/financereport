...
<?php 
	...
	for($i = 0; $i < sizeof($this->request->data['Alert']); $i++){
?>
	<fieldset>
...
//			echo $this->Form->input('Entity.'.$i.'.ticker',array('label'=>'Ticker','default'=>$tickers1[$this->request->data['Alert'][$i]['entity_id']]));
			...
			/*if ($this->request->data['Alert'][$i]['fired'] == 0)
				echo $this->Form->label($this->request->data['Alert'][$i]['fired']);
			else
	...
	function highlightNodes() {
		<?php for($i = 0; $i < sizeof($this->request->data['Alert']); $i++){ 
			echo 'highlightNode('.$this->request->data['Alert'][$i]['entity_id'].','.$i.');';
