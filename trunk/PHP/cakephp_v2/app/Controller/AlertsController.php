...
// Added the following line when troubleshooting the old bootstrap.php issue.
...
class AlertsController extends AppController {
	...
	function index() {
		...
		$filtervalues = $this->request->data;
			...
			/*if ($th['Alert']['schedule_id']!="All")
			 {
				...
				$conditions['Schedule.id'] = $this->request->data['Alert']['schedule_id'];
				}
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid alert'));
	...
	function add() {
		...
		if (!empty($this->request->data)) {
			...
			{
				//debug('here',true);
				$this->Session->setFlash(__('The alert cound not be saved. Please see below for additional information.'));
			}
			else
			{
				...
				$multiselect = $this->request->data['Alert']['entity_id'];
				$tmp2 = array();

				foreach ($multiselect as $item){
					$tmp = $this->request->data['Alert'];

					/*
					 *  Need to do a look up of the task_id based off of the entity_id and the metric_id
					 */

					$taskid = $this->getTaskId($item,$this->request->data['Alert']['metric_id']);
				...
				if ($this->Alert->saveAll($this->request->data['Alert'])) {
					//$this->Session->setFlash(__('The alert(s) have been saved'),'default',array('class'=>'green_message'));
					//$this->redirect(array('action' => 'index'));
					$num_of_alert = sizeof($this->request->data['Alert']);
					$this->Session->setFlash(__('The alert(s) have been saved'),'default',array('class'=>'green_message'));
	...
	function edit($id = null) {
		 ...
		 */
		if (array_key_exists('alert',$this->request->params['url']) && !empty($this->request->params['url']['alert']))
		{
			$record = "";
			$this->Session->delete('Record');
			$record= array(0 => $this->request->params['url']['alert']);
		}
		...
		if (!empty($this->request->data)) {
			...
			for($x=0;$x<sizeof($this->request->data['Alert']);$x++)
			{
				if (sizeof($this->request->data['Alert'][$x]['entity_id']) != 1 || empty($this->request->data['Alert'][$x]['entity_id'])) {
					$this->Session->setFlash(__('Please select one and only one entity id.'));
					//I want to return back to the edit screen.
					$this->redirect(array('action' => 'edit'),$this->request->data['Alert']['id']);
				}

				//$tmpticker = $this->request->data['Alert'][$x]['ticker'];
				//$tmpentityid = $this->Entity->find('first', array('fields'=> 'id','conditions'=>array('Entity.ticker'=>$tmpticker)));
				//$this->request->data['Alert'][$x]['entity_id'] = $tmpentityid['Entity']['id'];
				$this->request->data['Alert'][$x]['entity_id'] = $this->request->data['Alert'][$x]['entity_id'][0];
		...
		if (!empty($record)) {
			...
			$this->request->data = $this->toMulti($this->Alert->find('all',array('conditions'=>array('Alert.id'=>$record),'recursive'=>2)));
		}
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for alert'));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->Alert->delete($id)) {
			$this->Session->setFlash(__('Alert deleted'),'default',array('class'=>'green_message'));
			$this->redirect(array('action'=>'index'));
		}
...
function enquire() {
	if ( $this->request->data ) {
		$this->Alerts->set( $this->request->data );
