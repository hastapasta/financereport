...
class UsersController extends AppController {
	...
	function beforeFilter() {
		...
		$this->Auth->userScope = array(
	        'User.active' => array(
	            'expected' => 1,
	            'message' => __("Your account is not active yet. Click the Link in our Mail.")
	        )
	    );
	...
	function add() {
		if (!empty($this->request->data)) {
			...
			if ($this->User->save($this->request->data)) {
				$this->Session->setFlash(__('The user has been saved'),'default',array('class'=>'green_message'));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('The user cound not be saved. Please see below for additional information.'));
			}
		}
	...
	function edit($id = null) {
		...
		if (!empty($this->request->data)) {
			for($i = 0; $i < sizeof($this->request->data['User']); $i++){
				$this->request->data['User'][$i]['password'] = $this->Auth->password($this->request->data['User'][$i]['password']);
			}
		...
		if (!empty($record)) {
			//$this->request->data = $this->toMulti($this->User->find('all',array('conditions'=>array('User.id'=>$record))));
			...
			{
				$this->request->data = $this->toMulti($this->User->find('all',array('conditions'=>array('User.id'=>$userprops['User']['id']))));
	...
	function delete($id = null) {
		...
		if ($this->User->delete($id)) {
			$this->Session->setFlash(__('User deleted'),'default',array('class'=>'green_message'));
			$this->redirect(array('action'=>'index'));
		}
	...
	function change_password(){
		if(!empty($this->request->data)){
			...
			if($user && $user['User']['password'] != $this->Auth->password($this->request->data['User']['old_password']) ){
			...
			if($cansave){
				$user['User']['password' ] = $this->Auth->password($this->request->data['User']['new_password']);
				if($this->User->save($user)){
					$this->Session->setFlash(__('Password is changed successfully' ),'default',array('class'=>'green_message'));
				}else{
...
function registration() {
		if(!empty($this->request->data)){
			...
			/*if (sizeof($this->User->find('all',array('conditions'=>array('User.email'=>$this->request->data['User']['email'])))))
			{
				$this->Session->setFlash('An account for this email address already exists.');
			}*/
			if (sizeof($this->User->find('all',array('conditions'=>array('User.username'=>$this->request->data['User']['username'])))))
			{
				$this->Session->setFlash('Username already exists.');
			}
			else if(!($this->request->data['User']['password'] == $this->Auth->password($this->request->data['User']['confirm_password']))){
			...
			else
			{
				...
				if($this->User->save($this->request->data)){
					...
					$data['password'] = $this->request->data['User']['confirm_password'];

					$options['to'] = $this->request->data['User']['email'];
