...
class AppController extends Controller {
    ...
    function beforeFilter() {
        ...
        // The following line looks for an ActionsAuthorize.php
	...
	function _pageCounter() {
		...
		$query1 = "select * from page_counters";

		$result2 = mysql_query ("select * from page_counters");
	...
	/*function urlToNamed() {
        $urlArray = $this->request->params['url'];
        unset($urlArray['url']);
        if(!empty($urlArray)){
 ...
 */
	...
	*/
	...
	function action_process(){
		 ...
		 */
		if(1 == $this->request->data[$this->modelClass]['action_value']){
			unset($this->request->data[$this->modelClass]['action_value']);
			$this->delete_multiple($this->request->data);
		}
		/*
		 * code of 2 for edit
		 */
		if(2 == $this->request->data[$this->modelClass]['action_value']){

			unset($this->request->data[$this->modelClass]['action_value']);
			$ids = array();
			//debug($record,true);
			 foreach($this->request->data[$this->modelClass] as $record) {
				if($record != 0)
					$ids[] = $record;
			}
		 ...
		 *  code 3 for filter
		 */
		if(3 == $this->request->data[$this->modelClass]['action_value_search']){

			unset($this->request->data[$this->modelClass]['action_value_search']);
			debug('in action_process', true);

			$this->Session->write('FilterValues', $this->request->data);
