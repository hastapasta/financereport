...
class SchedulesController extends AppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid schedule'));
	...
	function edit($id = null) {
		...
		if (!empty($this->request->data)) {
			if ($this->Schedule->saveAll($this->request->data['Schedule'])) {
				$this->Session->setFlash(__('The schedule has been saved'),
				    'default',array('class'=>'green_message'));
		...
		if (!empty($record)) {
			$this->request->data = $this->toMulti($this->Schedule->find('all',
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for schedule'));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->Schedule->delete($id)) {
			$this->Session->setFlash(__('Schedule deleted'),'default',
			    array('class'=>'green_message'));
			$this->redirect(array('action'=>'index'));
		}
	...
	public function set_default_schedules() {
		...
		$this->Session->setFlash(__('Set all schedules to default configuration'),'default',array('class'=>'green_message'));
		$this->redirect(array('action' => 'index'));
	}
}
?>