...
function writelogdata($the_string)
{
...
writelog3('in Visualization.php');
...
class MC_Google_Visualization {
    ...
    public function __construct($db=null, $dialect='mysql') {
        ...
        $this->setDB($db);
        writelog3("before setsqldialect");
    ...
    public function generateMetadata($query) {
        ...
        writelog3('before return in generateMetadata');
        return $meta;
    }
    ...
    public function parseQuery($str) {
    	writelog3('in parsequery');
    	writelog3($str);
        $query = array();
        writelog3('parsequery 3');
       
       $tokens = $this->getGrammar()->parse($str);
       writelog3('parsequery 4');
        foreach($tokens->getChildren() as $token) {
        	writelog3($token->name);
            switch($token->name) {
                case 'select':
