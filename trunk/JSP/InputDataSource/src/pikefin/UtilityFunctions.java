...
public class UtilityFunctions {
	...
	public static HashMap<String,String> convertResultSetToHashMap(ResultSet rs, String col1, String col2) throws SQLException {
	/*
	 * Developed this function specifically for velocity. We want
	 * to use the value for one column (entity_id) as the property
	 * name for another column (value).
	 */
		...
		while (rs.next()) {
			//Velocity properties have to start with alpha-character so we'll prepend the column name
			hm.put(col1 + rs.getInt(col1), rs.getBigDecimal(col2).toString());
		}
	...
	public static ArrayList<HashMap<String,String>> convertResultSetToArrayList(ResultSet rs,String strUnique,boolean bIncludeTableName) throws SQLException {
		
		
		/*
		 * Velocity doesn't allow periods in variable names so we had to change the name 
		 * structure of the hash keys used in the hash values. Instead of <tablename>.<column>
		 * we're just going to go with <column>.
		 */
		...
		while (rs.next())
		{
			...
			for (int i=1;i<=numberOfColumns;i++)
			{
				...
				String strKey = null;
				/*
				 * Note: we are using underlying column name for <table name>.<column name> syntax
				 * and we are using column alias (i.e. column label as defined in AS clause) otherwise.
				 */
				if (bIncludeTableName)
					strKey= rsMetaData.getTableName(i)+"."+rsMetaData.getColumnName(i);
				else
					strKey = rsMetaData.getColumnLabel(i);
				
				if (strUnique != null) {
					if (strKey.equalsIgnoreCase(strUnique)) {
