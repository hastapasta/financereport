...
public class PikefinUtil{
	...
	public static int regexSeekLoop(String regex, int nCount, int nCurOffset, 
	    String strHayStack) throws TagNotFoundException {
	  ...
	  for (int i = 0; i < nCount; i++) {
	    ...
	    ApplicationSetting.getInstance().getStdoutwriter()
	      .writeln("regex iteration " + i	+ ", offset: " + nCurOffset,
	          Logs.STATUS2, "UF3");
	  }
	  return (nCurOffset);
	}
	...
	public static String regexSnipValue(String strBeforeUniqueCode,
	  ...
	  if ((strBeforeUniqueCode != null)
	    ...
	    ApplicationSetting.getInstance().getStdoutwriter()
	      .writeln(strBeforeUniqueCodeRegex, Logs.STATUS2, "UF4");
	  ...
	  ApplicationSetting.getInstance().getStdoutwriter()
	    .writeln("begin offset: " + nBeginOffset, Logs.STATUS2, "UF7");
	...
	public static String shortenURL(String strInputURL)	{
		try	{
			Url url = as("pikefin", "R_f08326b12abd18288243b65ef2c71c40")
			    .call(shorten(strInputURL));
			return(url.getShortUrl());
		} catch (Exception ex) {
			ApplicationSetting.getInstance().getStdoutwriter()
			  .writeln("Failed to shorten url: " + strInputURL,Logs.ERROR,"UF51.5");
