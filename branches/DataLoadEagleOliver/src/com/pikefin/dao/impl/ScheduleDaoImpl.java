...
@Component
public class ScheduleDaoImpl extends AbstractDao<Schedule> 
  implements ScheduleDao {
	...
	@Override
	// Saves the Schedule information into the database, it runs 
	// into a transaction
	// @author Amar_Deep_Singh
	...
	public Schedule saveScheduleInfo(Schedule scheduleEntity) 
	  ...
	  try{
	    ...
	    throw new GenericException(
	        ErrorCode.COULD_NOT_SAVE_SCHEDULE_DATA,
	...
	public Schedule updateScheduleInfo(Schedule scheduleEntity) 
	  ...
	  try {
	    ...
	    throw new GenericException(
	        ErrorCode.COULD_NOT_UPDATE_SCHEDULE_DATA,
	        e.getMessage(),e.getCause());
	  }
	  return scheduleEntity;
	}
	...
	public Boolean deleteScheduleInfo(Schedule scheduleEntity) 
	  ...
	  try{
	    ...
	    throw new GenericException(
	        ErrorCode.COULD_NOT_DELETE_SCHEDULE_INFORMATION,
	        e.getMessage(),e.getCause());
	  }
	  return result;
	}
	...
	@Override
	// Deleted the Schedule information from the database based on supplied 
	// ScheduleID. It first loads the Schedule then try to delete
	// @author Amar_Deep_Singh
	...
	@Override
	// Retrieve the Schedule information from the database based on supplied 
	// ScheduleID.
  // @author Amar_Deep_Singh
	// @param takes the ScheduleId of the Schedule entity
  // @returns returns Schedule entity if the Schedule is found else throws 
	// exception
	// @throws GenericException
	@Transactional(propagation=Propagation.REQUIRED)
	public Schedule loadScheduleInfo(Integer scheduleId) throws GenericException {
	  ...
	  try{
	    ...
	    throw new GenericException(ErrorCode.COULD_NOT_LOAD_SCHEDULE_DATA_WITH_ID,
	        e.getMessage(),e.getCause());
	  }
	  return scheduleEntity;
	}
	...
	public List<Schedule> loadAllSchedules() throws GenericException {
		...
		try{
			...
			schedules=(List<Schedule>)criteria.list();
				...
				throw new GenericException(ErrorCode.COULD_NOT_LOAD_REQUIRED_DATA,
				    e.getMessage(), e.getCause());
		}catch (Exception e) {
				throw new GenericException(ErrorCode.COULD_NOT_LOAD_REQUIRED_DATA,
				    e.getMessage(), e.getCause());
	...
	public List<Schedule> loadAllSchedulesForBrokerExecutor()
		...
		try{
			...
			Session session=getOpenSession();
			Criteria criteria=session.createCriteria(Schedule.class);
			Criterion  repeatTypeRunOnceCriteria=Restrictions.eq("type", 
			    RepeatTypeEnum.RUNONCE.toString());
			Criterion  repeatTypeRunEveryCriteria=Restrictions.eq("type", 
			    RepeatTypeEnum.RUNEVERY.toString());
			Criterion  repeatTypeDateCriteria=Restrictions.lt("nextTrigger", 
			    currentTime.getTime());
			Criterion  repeatTypeNotNoneCriteria=Restrictions.ne("type", 
			    RepeatTypeEnum.NONE.toString());
			criteria=criteria.createCriteria("repeatType")
			    .add(Restrictions.or(repeatTypeRunOnceCriteria,
			        repeatTypeRunEveryCriteria,repeatTypeDateCriteria))
			        .add(Restrictions.and(repeatTypeNotNoneCriteria));
			schedules=(List<Schedule>)criteria.list();
		}
		catch (HibernateException e) {
		  throw new GenericException(ErrorCode.COULD_NOT_LOAD_REQUIRED_DATA,
