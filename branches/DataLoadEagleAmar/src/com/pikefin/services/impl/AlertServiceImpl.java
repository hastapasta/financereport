...
public class AlertServiceImpl implements AlertService{
	...
	public void checkAlerts(DataGrabExecutor dataGrab) throws GenericException {
		...
		if(currentTask != null) {
		    ...
		    if(factDataMap == null || factDataMap.size()==0)  {
	           ApplicationSetting.getInstance().getStdoutwriter().writeln
	                ("No fact_data entries found for task " + currentTask  +
	                		". Alert processing terminated.",Logs.WARN,"A4.0");
	           return;
	        }
		 ...
		 if(!factDataMap.containsKey(key)){
			ApplicationSetting.getInstance().getStdoutwriter().writeln("No recent" +
					" fact data collected for ticker: " + tempAlert.getAlertEntity().getTicker()  +
					",alert id: "+ tempAlert.getAlertId() + ",entity id: " + 
					tempAlert.getAlertEntity().getEntityId() + ". Skipping alert" +
					" processing",Logs.WARN,"A2.7358");
			continue;
		 }
		...
		if (tempAlert.getAlertInitialFactData()==null || 
				tempAlert.getAlertInitialFactData().getValue()==null) {
		...
		if(tempAlert.getType()==1) {
		  if((calObservationPeriodBegin.after(calInitialFactDataCollected) &&
				  bAutoResetPeriod) || (bPopulateInitialFactDataId == true)) {
		  ...
		  if(bPopulateInitialFactDataId == true) {
			...
			if(bdCurrent.equals(bdTarget)) {
			  ApplicationSetting.getInstance().getStdoutwriter().writeln("Attempting " +
			  		"to populate initial_fact_data_id with same value as target " +
			  		"(alert type 2, alert id: " + tempAlert.getAlertId() + "). " +
			  				"Skipping Alert.",Logs.WARN,"A10.324");
			 continue;
			}
			newAlertCopy.setAlertInitialFactData(rsFactData);
			newAlertCopy.setNotificationCount(0);
		...
		if(this.getTriggered(tempAlert,factDataMap,key)) {
		  ...
		  for(AlertTarget tempTarget:alertTargets){
		    switch(tempTarget.getType()){
			  case 1:// Email notification.
				...
				if(!tempTarget.getBulkEmail()){
				  if(nNotificationCount<nMaxNotifications){
					...
					if(tempAlert.getAlertEntity().getFullName()!= null)
					  strMsg = strMsg + "Full Description: " + tempAlert.
					  getAlertEntity().getFullName() +" \r\n";
					else
					  strMsg = strMsg + "Full Description: \r\n";
					strMsg = strMsg + "Task: " + currentTask.getDescription() +
							"\r\n";
					strMsg = strMsg + "Amount: " + dChange.multiply(new BigDecimal(100))
							.toString() + "%\r\n";
					strMsg = strMsg + "Current Value : " +factDataMap.get(key).
							getValue()+ "\r\n";
					strMsg = strMsg + "Previous Value: " +tempAlert.
							getAlertInitialFactData().getValue()+ "\r\n";
					SimpleDateFormat sdf = new SimpleDateFormat("MMM d,yyyy HH:mm:ss Z");
					strMsg = strMsg + "Current Timestamp : " + 
					sdf.format(calJustCollected.getTime()) + "\r\n";
					strMsg = strMsg + "Previous Timestamp: " +
					sdf.format(calInitialFactDataCollected.getTime()) + "\r\n";
					strMsg += "Observation Period End: " + 
					sdf.format(calObservationPeriodEnd.getTime()) + "\r\n";
					strMsg += "Observation Period Begin: " + 
					sdf.format(calObservationPeriodBegin.getTime()) + "\r\n";
					strMsg += "Chart: " + ApplicationSetting.getInstance().
						getEmailPhpUrl() +"charts/allassets/linechart.php?a=" +
						tempAlert.getAlertId() + "\r\n";
					//Add link for Increase Alert Limit form
					strMsg = strMsg + "Modify/View Alert Properties: " +
					ApplicationSetting.getInstance().getEmailCakeUrl() +
					"alerts/edit?alert=" + tempAlert.getAlertId();
					String strSubject = ApplicationSetting.getInstance().
					getEmailSubjectText() + " : " + strTicker + " : " +
					tempAlert.getAlertTimeEvent().getName() + " Observation Period";
				...
				if(tempAlert.getType()== 1) {
				  ...
				  String strUrl = ApplicationSetting.getInstance().getEmailPhpUrl() 
					+ "charts/allassets/";
				  if(tempAlert.getAlertTimeEvent().getTimeEventId().equals("1")
					   || tempAlert.getAlertTimeEvent().getTimeEventId().equals("3"))
					 strUrl += "linechart15min.php";
				  else
					 strUrl += "linechart.php";
				  strUrl += "?a=" + tempAlert.getAlertId();
				  strTweet += " " + PikefinUtil.shortenURL(strUrl);
				  // Figure out if this alert represents an actual stock ticker.
				  // If so, then use the $ tag; otherwise use the # tag.
				  List<EntityGroup> entityGroupList= entityGroupService.
						  loadEntityGroupsByEntity(tempAlert.getAlertEntity());
				  if (entityGroupList!=null && entityGroupList.size()!=0) {
					...
					/* Forex */
					  ...
					  case 5:
						...
						if(entityCountries!=null && entityCountries.get(0)!=null && 
								entityCountries.get(0).getHash()!= null && 
								entityCountries.get(0).getHash().trim().length()>0) {
					...
					if(tempAlert.getAlertEntity().getHash() != null && 
							tempAlert.getAlertEntity().getHash().trim().length()>0) {
			...
			if(nTweetCount >= nTweetLimit && !(nTweetLimit<0)){
			  strErrorMsg = "Tweet not sent because internal tweet limit of " +
			nTweetLimit + " exceeded.";
			}else{
			  //Debug mode is handled inside the tweet function
			  if(tempTarget.getUserId() !=null  && 
					  tempTarget.getSecurityAccountId()!=null){
			    strErrorMsg = notificationService.tweet(strTweet,
			    tempTarget.getUserId().getUsername(),tempTarget.getUserId().getPassword(),
			    tempTarget.getSecurityAccountId().getAuth1(),
			    tempTarget.getSecurityAccountId().getAuth2(),
			    tempTarget.getSecurityAccountId().getAuth3(),
			    tempTarget.getSecurityAccountId().getAuth4());
				try {
					  //Need a delay for unlimited tweeting.
					  if(nTweetLimit<0){
					     ApplicationSetting.getInstance().getStdoutwriter().
					     writeln("Unlimited tweeting. Pausing for 30 seconds..."
					    		 ,Logs.STATUS2,"A4.63");
						 Thread.sleep(30000);
					  }
			...
			try{
			  ...
			  ApplicationSetting.getInstance().getStdoutwriter().
			  writeln("Issue inserting into log_tweets "+sqle.getErrorCode()+" "+
			  sqle.getErrorDescription(),Logs.ERROR,"A4.6");
		 ...
		 default:
		  //Invalid target code
		  ApplicationSetting.getInstance().getStdoutwriter().
		  writeln("Invalid alert target code.",Logs.ERROR,"A4.72");
		    break;
	   }
	}
	...
	public Map<TheKey,FactData> convertListToFactDataMap(List<FactData> factDataList) throws GenericException{
		...
		for(FactData factEntity:factDataList){
			if(factEntity.getEntity()==null || factEntity.getEntity().getEntityId() == null) {
				ApplicationSetting.getInstance().getStdoutwriter().writeln("Problem " +
						"converting List<FactData> to Map - column for hash key" +
						" not found",Logs.ERROR,"UF18.7");
				throw new GenericException("Custom  Exception - Unable to convert" +
						" List<FactData> to HashMap");
			}
			TheKey key=new TheKey(factEntity.getEntity().getEntityId(), 
					factEntity.getFiscalyear());
...
private boolean getTriggered(Alert alertDetails,Map<TheKey,FactData> factDataMap,TheKey key) {
    ...
    switch (alertDetails.getType()) {
	  ...
	  case 3:
	    ...
	    if(dJustCollectedValue.equals(new Double("0.000")) || 
	    		dJustCollectedValue.equals(new Double("0.00")))  {
		  ApplicationSetting.getInstance().getStdoutwriter().
		  writeln("Zero was the last value collected. Probably a bad print. " +
		  "Skipping ticker " + alertDetails.getAlertEntity().getTicker(),Logs.WARN,"A2.7385");
		  return false;
		}
	try{
	  dChange = dJustCollectedValue.subtract(dInitialFactDataValue).divide(dInitialFactDataValue,BigDecimal.ROUND_HALF_UP);
	}catch (ArithmeticException ae) {
	  ApplicationSetting.getInstance().getStdoutwriter().writeln("Error doing " +
	  		"BigDecimal arithmetic",Logs.ERROR,"A2.7385");
	  ApplicationSetting.getInstance().getStdoutwriter().writeln("Error occured " +
	  		"processing alert id: " + alertDetails.getAlertId(),Logs.ERROR,"A2.7386");
