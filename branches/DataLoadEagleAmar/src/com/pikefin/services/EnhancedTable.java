...
public class EnhancedTable {
	...
	public ArrayList<String[]> enhancedGetTable() throws DataAccessException, TagNotFoundException,	PrematureEndOfDataException {
		...
		if (Constants.EnhancedTableSection.ENHANCED_TABLE_BODY_SECTION.equals(strSection)) {
			currentExtractTable = j.getExtractKeyBody();
		} else if (Constants.EnhancedTableSection.ENHANCED_TABLE_COL_HEAD_SECTION.equals(strSection)) {
			currentExtractTable = j.getExtractKeyColhead();
		} else if (Constants.EnhancedTableSection.ENHANCED_TABLE_ROW_HEAD_SECTION.equals(strSection)) {
		...
		for (int nCurTable = nBeginTable; nCurTable <= nEndTable; nCurTable++) {
			...
			ApplicationSetting.getInstance().getStdoutwriter().writeln("Searching table count: "
			...
			int nEndRowOffset = PikefinUtil.regexSeekLoop("(?i)(</tr>)",
					1, nCurOffset,dg.returned_content);
		
			int nEndTableOffset = PikefinUtil.regexSeekLoop("(?i)(</TABLE>)", 1, nCurOffset,dg.returned_content);
			...
			while (!done) {
				...
				try {
						...
						nEndRowOffset = PikefinUtil.regexSeekLoop("(?i)(</tr>)",
								1, nCurOffset,dg.returned_content);
	...
	private String[] enhancedProcessRow(String strRow) throws DataAccessException {
		...
		String[] rowdata;
		ApplicationSetting.getInstance().getStdoutwriter().writeln("row: " + nRowCount,
				Logs.STATUS2, "DG28");
		...
		try {
			...
			//while ()
				...
				if (bColTHTags == false)
					...
					nRowOffset = PikefinUtil.regexSeekLoop("(?i)(<td[^>]*>)",col.getColPosition(), nRowOffset,strRow);
				else
				...
				try {
					...
					ApplicationSetting.getInstance().getStdoutwriter().writeln("Invalid or Irregularly formed table row, inserting pikefinvoid.",Logs.WARN, "DG30.999");
			...
			ApplicationSetting.getInstance().getStdoutwriter().writeln("Invalid or Irregularly formed table row, inserting pikefinvoid.",Logs.WARN, "DG30.27");
			rowdata[0] = "pikefinvoid";
			return(rowdata);
		}
	}
