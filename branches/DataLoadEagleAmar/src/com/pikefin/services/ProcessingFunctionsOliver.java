...
*/
  ...
  */
  ...
  public void postProcessGasolineEurope() {
    ...
    while (scanner.hasNextLine()) {
      ...
      strQuery += " join countries_entities on countries_entities.entity_id" +
          " = entities.id ";
      strQuery += " left join country_aliases on country_aliases.country_id" +
          " = countries_entities.country_id ";
      strQuery += " left join countries on countries.id" +
          " = countries_entities.country_id ";
      strQuery += " where ticker='macro'";
      strQuery += " and (alias='" + strCountry + 
          "' OR countries.name='" + strCountry + "') ";
  ...
  */
  ...
  public void postProcessWikipediaGasoline() throws SQLException {
    ...
    int nCounter=0;
    // This is kludgy but don't know of a better way to do this atm. With the 
    // way we are currently reading in countries, we get two identical Vietnams
    // and we only want the first one.
    // 
    // Actually the specialized vietname code isn't even being used right now
    // because the 2nd vietnam generates a pikefinvoid because of the missing
    ...
    for (String[] rowdata : propTableData)  {
      ...
      // 2/25/2012 - Right now one of the dates (Oman) has octane info in the
      // cell as well.
